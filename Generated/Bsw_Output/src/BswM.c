...
/* polyspace-begin RTE:OBAI [Justified:Medium] "Pointer not access out of array" */
/* polyspace-begin RTE:UNR [Justified:Medium] "Defensive programming" */
...
**                          pRuleFunction                                     **
*******************************************************************************/
/* polyspace +3 MISRA-C3:8.3 [Justified:Medium] "Adopted code integration(NvM)" */
...
**                        current state of the Client Service                 **
**                        (available/down).                                   **
**                                                                            **
** Reentrancy           : Reentrant                                           **
**                                                                            **
...
{
  #if (BSWM_TOTAL_PORTS_OF_SD_CLIENTSERVICE_CURRENTSTATE > BSWM_ZERO)
  uint8 LucId;
  BswM_RuleIdType LddId;
  P2CONST(BswM_RuleIdType, AUTOMATIC, BSWM_CONST) LpRuleId;
  ...
  {
    /* Report to DET */
    (void)Det_ReportError(BSWM_MODULE_ID, BSWM_ZERO, BSWM_SD_CLIENTSERVICECURRENTSTATE_SID,
    BSWM_E_NO_INIT);
  }
  else
  #endif
  {
    #if (BSWM_DEV_ERROR_DETECT == STD_ON)
    if ((SD_CLIENT_SERVICE_DOWN != CurrentClientState) &&
        (SD_CLIENT_SERVICE_AVAILABLE != CurrentClientState))
    {
      (void)Det_ReportError(BSWM_MODULE_ID, BSWM_ZERO,
        BSWM_SD_CLIENTSERVICECURRENTSTATE_SID, BSWM_E_REQ_MODE_OUT_OF_RANGE);
    }
    else
    #endif /* #if (BSWM_DEV_ERROR_DETECT == STD_ON) */
    {
      /* Module is initialized */
      for (LucId = BSWM_ZERO; LucId < BSWM_TOTAL_PORTS_OF_SD_CLIENTSERVICE_CURRENTSTATE; LucId++)
      {
        /* If Handle ID matches */
        if (BswM_GaaSdClientServiceCurrentStateProperty[LucId].usSdClientServiceHandleId == SdClientServiceHandleId)
        {
          /* Enter in Data Protection */
          SchM_Enter_BswM_DATA_PROTECTION();
          BswM_GaaSdClientServiceCurrentState[LucId].ddCurrentMode = (uint16)CurrentClientState;
          BswM_GaaSdClientServiceCurrentState[LucId].blModeValueStatus = BSWM_VALID;
          SchM_Exit_BswM_DATA_PROTECTION();

          LpRuleId = BswM_GaaSdClientServiceCurrentStateProperty[LucId].pRuleId;

          for (LddId = BSWM_ZERO; LddId < BswM_GaaSdClientServiceCurrentStateProperty[LucId].ddTotalRules; LddId++)
          {
            if (BSWM_FALSE ==
                BswM_GaaRuleInitProperty[LpRuleId[LddId]].blNestedExecutionOnly)
            {
               ...
               */
              if (BSWM_DEFERRED ==
                  BswM_GaaSdClientServiceCurrentStateProperty[LucId].blRequestProcessing)
              {
                /* Enter in Data Protection */
                SchM_Enter_BswM_RULESTATUS_PROTECTION();
                /* ucRuleResult, blRuleStatus */
...
{
  #if (BSWM_TOTAL_PORTS_OF_SD_CONSUMEDEVENTGROUP_CURRENTSTATE > BSWM_ZERO)
  uint8 LucId;
  BswM_RuleIdType LddId;
  P2CONST(BswM_RuleIdType, AUTOMATIC, BSWM_CONST) LpRuleId;
  ...
  {
    /* Report to DET */
    (void)Det_ReportError(BSWM_MODULE_ID, BSWM_ZERO, BSWM_SD_CONSUMEDEVENTGROUPCURRENTSTATE_SID,
    BSWM_E_NO_INIT);
  }
  else
  #endif
  {
    #if (BSWM_DEV_ERROR_DETECT == STD_ON)
    if ((SD_CONSUMED_EVENTGROUP_DOWN != ConsumedEventGroupState) &&
        (SD_CONSUMED_EVENTGROUP_AVAILABLE != ConsumedEventGroupState))
    {
      (void)Det_ReportError(BSWM_MODULE_ID, BSWM_ZERO,
        BSWM_SD_CONSUMEDEVENTGROUPCURRENTSTATE_SID, BSWM_E_REQ_MODE_OUT_OF_RANGE);
    }
    else
    #endif /* #if (BSWM_DEV_ERROR_DETECT == STD_ON) */
    {
      /* Module is initialized */
      for (LucId = BSWM_ZERO; LucId < BSWM_TOTAL_PORTS_OF_SD_CONSUMEDEVENTGROUP_CURRENTSTATE; LucId++)
      {
        /* If Handle ID matches */
        if (BswM_GaaSdConsumedEventGroupCurrentStateProperty[LucId].usSdConsumedEventGroupHandleId == SdConsumedEventGroupHandleId)
        {
          /* Enter in Data Protection */
          SchM_Enter_BswM_DATA_PROTECTION();
          BswM_GaaSdConsumedEventGroupCurrentState[LucId].ddCurrentMode = (uint16)ConsumedEventGroupState;
          BswM_GaaSdConsumedEventGroupCurrentState[LucId].blModeValueStatus = BSWM_VALID;
          SchM_Exit_BswM_DATA_PROTECTION();

          LpRuleId = BswM_GaaSdConsumedEventGroupCurrentStateProperty[LucId].pRuleId;

          for (LddId = BSWM_ZERO; LddId < BswM_GaaSdConsumedEventGroupCurrentStateProperty[LucId].ddTotalRules; LddId++)
          {
            if (BSWM_FALSE ==
                BswM_GaaRuleInitProperty[LpRuleId[LddId]].blNestedExecutionOnly)
            {
               ...
               */
              if (BSWM_DEFERRED ==
                  BswM_GaaSdConsumedEventGroupCurrentStateProperty[LucId].blRequestProcessing)
              {
                /* Enter in Data Protection */
                SchM_Enter_BswM_RULESTATUS_PROTECTION();
                /* ucRuleResult, blRuleStatus */
...
{
  #if (BSWM_TOTAL_PORTS_OF_SD_EVENTHANDLER_CURRENTSTATE > BSWM_ZERO)
  uint8 LucId;
  BswM_RuleIdType LddId;
  P2CONST(BswM_RuleIdType, AUTOMATIC, BSWM_CONST) LpRuleId;
  ...
  {
    /* Report to DET */
    (void)Det_ReportError(BSWM_MODULE_ID, BSWM_ZERO, BSWM_SD_EVENTHANDLERCURRENTSTATE_SID,
    BSWM_E_NO_INIT);
  }
  else
  #endif
  {
    #if (BSWM_DEV_ERROR_DETECT == STD_ON)
    if ((SD_EVENT_HANDLER_RELEASED != EventHandlerStatus) &&
        (SD_EVENT_HANDLER_REQUESTED != EventHandlerStatus))
    {
      (void)Det_ReportError(BSWM_MODULE_ID, BSWM_ZERO,
        BSWM_SD_EVENTHANDLERCURRENTSTATE_SID, BSWM_E_REQ_MODE_OUT_OF_RANGE);
    }
    else
    #endif /* #if (BSWM_DEV_ERROR_DETECT == STD_ON) */
    {
      /* Module is initialized */
    for (LucId = BSWM_ZERO; LucId < BSWM_TOTAL_PORTS_OF_SD_EVENTHANDLER_CURRENTSTATE; LucId++)
      {
        /* If Handle ID matches */
        if (BswM_GaaSdEventHandlerCurrentStateProperty[LucId].usSdEventHandlerHandleId == SdEventHandlerHandleId)
        {
          /* Enter in Data Protection */
          SchM_Enter_BswM_DATA_PROTECTION();
          BswM_GaaSdEventHandlerCurrentState[LucId].ddCurrentMode = (uint16)EventHandlerStatus;
          BswM_GaaSdEventHandlerCurrentState[LucId].blModeValueStatus = BSWM_VALID;
          SchM_Exit_BswM_DATA_PROTECTION();

          LpRuleId = BswM_GaaSdEventHandlerCurrentStateProperty[LucId].pRuleId;

          for (LddId = BSWM_ZERO; LddId < BswM_GaaSdEventHandlerCurrentStateProperty[LucId].ddTotalRules; LddId++)
          {
            if (BSWM_FALSE ==
                BswM_GaaRuleInitProperty[LpRuleId[LddId]].blNestedExecutionOnly)
            {
               ...
               */
              if (BSWM_DEFERRED ==
                  BswM_GaaSdEventHandlerCurrentStateProperty[LucId].blRequestProcessing)
              {
                /* Enter in Data Protection */
                SchM_Enter_BswM_RULESTATUS_PROTECTION();
                /* ucRuleResult, blRuleStatus */
