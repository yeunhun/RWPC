...
**                                                                            **
** 1.6.4    04-09-2020   EunKyung.Kim    Changes made As per Redmine #25254   **
**                                                                            **
** 1.6.3    01-06-2020    EunKyung.Kim    Changes made As per Redmine #19808, **
**                                        #20595                              **               
**                                                                            **
** 1.6.2    02-04-2019    EunKyung.Kim    Changes made As per Redmine #16812  **
**                                                                            **
** 1.6.1    31-01-2019    EunKyung.Kim    Changes made As per Redmine #14481, **
**                                        #15936, #15990                      **
**                                                                            **
** 1.6.0    14-12-2018    EunKyung.Kim    Changes made As per Redmine #8561   **
**                                                                            **
** 1.5.6    12-12-2018    YoungJin.Yun    Changes made As per Redmine #14900  **
**                                                                            **
** 1.5.5    09-11-2018    EunKyung.Kim    Changes made As per Redmine #14481  **
**                                                                            **
...
/* polyspace-begin DEFECT:NULL_PTR [Justified:Low] "Not a defect" */
/*******************************************************************************
**                      Global Data Types                                     **
*******************************************************************************/
 ...
 * Service ID           : None
 *
 * Description          : Sets Event ID to Event memory. 
 *
 * Sync/Async           : Synchronous
 *
 * Re-entrancy          : Reentrant
 *
 * Input Parameters     : EmEntry, EventId
 *                        
 ...
 * Service ID           : None
 *
 * Description          : Clear All Event data in event memory. 
 *
 * Sync/Async           : Synchronous
 *
 * Re-entrancy          : Reentrant
 ...
 * Service ID           : None
 *
 * Description          : Get Event memory pointer by memory position. 
 *
 * Sync/Async           : Synchronous
 *
 * Re-entrancy          : Reentrant
...
{  
  ...
  case DEM_DTC_ORIGIN_PERMANENT_MEMORY:
    /* if (MemLocation < Dem_SizeOfEventMemory[DTCOrigin]) */
    {
      memEntry = &Dem_PermanentEventMemory[MemLocation];
    }
    break;
  #endif
  #endif
 ...
 * Service ID           : None
 *
 * Description          : Get Event ID by event memory location. 
 *
 * Sync/Async           : Synchronous
 *
 * Re-entrancy          : Reentrant
  ...
  const Dem_NumOfEventMemoryEntryType MemLocation)
{
  ...
  #if (DEM_MAX_NUMBER_EVENT_ENTRY_PER > 0U)    
  case DEM_DTC_ORIGIN_PERMANENT_MEMORY:
    eventId = Dem_PerDtcState[MemLocation].EventIdRef;
    break;    
  #endif
  #endif
  /* polyspace<RTE:UNR:Not a defect:No action planned> Default switch clause is unreachable. switch-expression. */    
...
FUNC(P2VAR(uint8, AUTOMATIC, DEM_VAR), DEM_CODE) Dem_EvMem_GetJ1939FFBuffer (
  Dem_EventRelatedDataCPCType EvData,
  Dem_EventMemoryEntryPtrType MemEntry,
  P2VAR(uint32, AUTOMATIC, DEM_VAR) BufSize, 
  Dem_J1939DcmSetFreezeFrameFilterType J1939FFtype)
{
  ...
  if((J1939FFtype == DEM_J1939DCM_EXPANDED_FREEZEFRAME) && (EvData->J1939FFClassPos < Dem_Num_Of_J1939FreezeFrameClass))
  {
...
FUNC(uint32, DEM_CODE) Dem_EvMem_GetTimeStamp( Dem_EventMemoryEntryPCType entry)
{
...
Dem_EvMem_SetFirstLastConfirmedDTC(Dem_DTCOriginType DTCOrigin, Dem_DTCType DTC)
{
  boolean dirty = DEM_FALSE;
  ...
  boolean state)
{
  ...
  if ((boolean)DEM_TRUE == Dem_EvMem_DTCOriginIsValid(DTCOrigin))
  {
    ...
    if (oldState != state)
    {
      Dem_EventStatusNvRamData.EventMemoryOverflow = (state == (boolean)DEM_TRUE) ? 
...
FUNC(void, DEM_CODE) Dem_EvMem_ResetEventStatusNvRamBlock(void)
{
  ...
  */
  ...
  for(nvDataIndex = 0u; nvDataIndex < DEM_BITARRAY_BITNSLOTS(1U+Dem_Num_Of_Indicator_Attribute_Status); nvDataIndex++)
  {    
  ...
  #if(DEM_OBD_SUPPORT == STD_ON)
  Dem_EventStatusNvRamData.DtcClear = 0;
  Dem_EventStatusNvRamData.NumOfWUCSinceLC = 0;
 
  #if(DEM_NUMBER_OF_RATIO_INFO > 0U)
  for(nvDataIndex = 0u; nvDataIndex < DEM_SIXTEEN; nvDataIndex++)
  {
    Dem_EventStatusNvRamData.IumprDenominator[nvDataIndex] = 0;
    Dem_EventStatusNvRamData.IumprNumerator[nvDataIndex] = 0;
  }
  Dem_EventStatusNvRamData.IgnitionCylCnt = 0;
  Dem_EventStatusNvRamData.GeneralDenCnt = 0;
  #endif
  #endif
...
{
  boolean dirty = DEM_FALSE;
  if ((NULL_PTR != entry) && (entry->AgingCycleCounter < (uint8)0xFF))
  {
...
{
  boolean dirty = DEM_FALSE;
  if ((NULL_PTR != entry) && (AgingCounter != entry->AgingCycleCounter))
  {
    dirty = DEM_TRUE;
  ...
  const boolean AttStatus)
{
  ...
  if (DEM_TRUE == AttStatus)
  {
    ...
    {
      /* polyspace +1 MISRA-C3:D4.1 [Justified:Low] "Not a defect" */	
      DEM_BITARRAY_BITSET(Dem_EventStatusNvRamData.AttributeStatus,\
  ...
  else
  {
    ...
    {
      /* polyspace +2 MISRA-C3:D4.1 [Justified:Low] "Not a defect" */	
      DEM_BITARRAY_BITCLEAR(Dem_EventStatusNvRamData.AttributeStatus, \
        IndicatorAtt->IndicatorAttStatusPos);
      dirty = DEM_TRUE;                      
    }
    #endif
  ...
  P2CONST(Dem_IndicatorAttributeType , AUTOMATIC, DEM_VAR)  IndicatorAttr)
{
  ...
  {
    /* polyspace +1 MISRA-C3:D4.1 [Justified:Low] "Not a defect" */  
    Dem_NumOfIndicatorAttributeType pos = IndicatorAttr->FailureCycleCounterPos;
    if (pos < Dem_Num_Of_Failure_Counter)
    {
      if (0U != Dem_EventStatusNvRamData.IndicatorFailureCycleCounter[pos])
      {
        Dem_EventStatusNvRamData.IndicatorFailureCycleCounter[pos] = 0U;
        dirty = DEM_TRUE;
      }
    }
  }
  ...
  #if (DEM_NUM_OF_HEALING_COUNTER > 0u)
  {
    Dem_NumOfIndicatorAttributeType pos = IndicatorAttr->HealingCycleCounterPos;
    if (pos < Dem_Num_Of_Healing_Counter)
    {
      if (0U != Dem_EventStatusNvRamData.IndicatorHealingCycleCounter[pos])
      {
        Dem_EventStatusNvRamData.IndicatorHealingCycleCounter[pos] = 0U;
        dirty = DEM_TRUE;
      }
    }
  }
  ...
  #if (DEM_NUM_OF_INDICATOR_ATTRIBUTE_STATUS > 0u)
  { 
    Dem_NumOfIndicatorAttributeType pos = IndicatorAttr->IndicatorAttStatusPos;  
    if (pos < Dem_Num_Of_Indicator_Attribute_Status)
    {
      /* polyspace +1 MISRA-C3:10.4 [Justified:Low] "Not a defect" */	
      if (DEM_FALSE != DEM_BITARRAY_BITTEST_ISBITSET(\
                        Dem_EventStatusNvRamData.AttributeStatus , pos))
      {
        DEM_BITARRAY_BITCLEAR(Dem_EventStatusNvRamData.AttributeStatus , pos);          
        dirty = DEM_TRUE;
      }
    }
  }
  ...
  #if((DEM_NUMBER_OF_INDICATORS > 0 ) && (DEM_NUM_OF_INDICATOR_BEHAVIOR > 0))
    /* polyspace +2 MISRA-C3:D4.1 [Justified:Low] "Not a defect" */
    Dem_IndicatorBehaviorAccumulator
...
Dem_EvMem_InitIndicatorBehaviorAccumulator(void)
{
  ...
  #if (DEM_NUM_OF_INDICATOR_ATTRIBUTE_STATUS > 0u)    
  Dem_NumOfIndicatorAttributeType pos;
  #endif
  
  P2CONST(Dem_IndicatorAttributeType , AUTOMATIC, DEM_VAR)  indicator;    
  Dem_NumOfIndicatorAttributeType idx;
  
  for (idx = 0; idx < Dem_Number_Of_Indicators ; idx++)
  {  
    indicator = &Dem_GaaIndicatorList[idx];
    
    #if (DEM_NUM_OF_INDICATOR_ATTRIBUTE_STATUS > 0u)
    /* polyspace +1 MISRA-C3:D4.1 [Justified:Low] "Not a defect" */
    pos = indicator->IndicatorAttStatusPos;  
    if (pos < Dem_Num_Of_Indicator_Attribute_Status)    
    {
      /* polyspace +3 MISRA-C3:10.8 [Justified:Low] "Not a defect" */	
      /* polyspace +2 MISRA-C3:10.5 [Justified:Low] "Not a defect" */	
      /* polyspace +1 MISRA-C3:10.3 [Justified:Low] "Not a defect" */
      status = (boolean)DEM_BITARRAY_BITGET(Dem_EventStatusNvRamData.AttributeStatus , pos);    
    }
    else
    #endif
    {
      /* polyspace +4 MISRA-C3:10.8 [Justified:Low] "Not a defect" */	
      /* polyspace +3 MISRA-C3:10.5 [Justified:Low] "Not a defect" */	
      /* polyspace +2 MISRA-C3:D4.1 [Justified:Low] "Not a defect" */
      /* polyspace +1 MISRA-C3:10.3 [Justified:Low] "Not a defect" */	
      status = (boolean)(((uint8)(Dem_AllEventStatusByte[indicator->EventIdRef] >> 7U)) & (uint8)0x01);
    }
    /* polyspace +1 MISRA-C3:10.4 [Justified:Low] "Not a defect" */
    if (DEM_FALSE != status)
    {
      /* polyspace +2 MISRA-C3:D4.1 [Justified:Low] "Not a defect" */	
      Dem_IndicatorBehaviorAccumulator
        [indicator->IndicatorIdRef][indicator->Behavior]++;      
    }
...
* Service ID           : None
*
* Description          : Retrieve indicator attribute status of the specific event
*
* Sync/Async           : Synchronous
*
* Re-entrancy          : Reentrant
  ...
  const Dem_NumOfIndicatorAttributeType IndicatorAttrId)
{
  ...
  {
    /* polyspace +2 MISRA-C3:10.3 [Justified:Low] "Not a defect" */  
    /* polyspace +1 MISRA-C3:D4.1 [Justified:Low] "Not a defect" */  
    status = DEM_BITARRAY_BITGET(Dem_EventStatusNvRamData.AttributeStatus , IndicatorAttrId);    
  }
  else
  #endif
  {
    /* polyspace +2 MISRA-C3:10.8 [Justified:Low] "Not a defect" */  
    /* polyspace +1 MISRA-C3:10.5 [Justified:Low] "Not a defect" */  
...
)
{
  ...
  if (Dem_EvMemToEventMap[EventId] <  Dem_SizeOfEventMemory[DTCOrigin])
  {
    ...
    if (NULL_PTR != memEntry) 
    {
      ...
      if (NULL_PTR != cbDTC)
      {
	      Dem_NumOfEventIdType evIndex = 0U;
        ...
        else
        {
          ...
          {    
            if ((evIndex < DEM_MAX_NUM_EVENTS_PER_DTC) && (Dem_ListOfCombinedEvents[startPos] == EventId))
            {
              udsStatusByte = memEntry->EventStatuByte[evIndex];
              break;
            }
            ++evIndex;
            ++startPos;
  ...
  if (NULL_PTR != evMemEntry)
  {
    ...
    {  
      ...
      else
      {
        ...
        while (startPos < endPos)
        {    
          if ((evIndex < DEM_MAX_NUM_EVENTS_PER_DTC) && 
            (Dem_ListOfCombinedEvents[startPos] == EventId))
          {
            evMemEntry->EventStatuByte[evIndex] = EventStatusByteNew;
            break;
          }
          ++evIndex;
          ++startPos;
  ...
  boolean EvStatusIsDirty)
{
  ...
  #endif
  {
    ...
    if ( memLoc < Dem_SizeOfEventMemory[DTCOrigin])
    {
      ...
      {
        mask = DEM_UDS_STATUS_MASK_EVMEM & ((Dem_UdsStatusByteType)((uint8)0xFFU & ~(Dem_UdsStatusByteType)DEM_UDS_STATUS_TF));
      }      
    ...
    {    
      ...
      if ((boolean)DEM_TRUE == storageTestFailed)
      {
        mask = DEM_UDS_STATUS_MASK_ALLEVENTSTATUS | ((Dem_UdsStatusByteType)DEM_UDS_STATUS_TF);
      }
      else /* FALSE */
      {
        mask = DEM_UDS_STATUS_MASK_ALLEVENTSTATUS & ((Dem_UdsStatusByteType)((uint8)0xFFU & ~(Dem_UdsStatusByteType)DEM_UDS_STATUS_TF));
      }  
    ...
    /* polyspace +1 MISRA-C3:D4.1 [Justified:Low] "Not a defect" */
    if (((boolean)DEM_TRUE == EvMemEntryIsDirty) && ( memLoc < Dem_SizeOfEventMemory[DTCOrigin]))
    {
      Dem_EvMem_SetEvMemNvBlockStatus(DTCOrigin, memLoc, NvDataBlkStatus);
    }
    
    if ((boolean)DEM_TRUE == EvStatusIsDirty)
    {
...
{        
  ...
  { 
    #if(DEM_EVENT_COMBINATION_SUPPORT == DEM_EVCOMB_TYPE1)  
    dtcConfirmed  = (uint8)(DTCStatusOld & (Dem_UdsStatusByteType)DEM_UDS_STATUS_CDTC);
    #else /* combination is disabled or type 2 */
    dtcConfirmed  = (uint8)(EventStatusByteOld & (Dem_UdsStatusByteType)DEM_UDS_STATUS_CDTC);
    #endif        
    ...
    {       
      ...
      if (event->e_PointToData < Dem_Num_Of_EventRelatedData)      
      {
        ...
        {
          if ((boolean)DEM_TRUE == Dem_StoreEventRelatedData(&Dem_EventRelatedData[event->e_PointToData], memEntry))
          {
            *EventDirty = DEM_TRUE;
          }
        }          
...
/* polyspace-end DEFECT:NULL_PTR [Justified:Low] "Not a defect" */
/*******************************************************************************
**                      End of File                                           **
*******************************************************************************/
