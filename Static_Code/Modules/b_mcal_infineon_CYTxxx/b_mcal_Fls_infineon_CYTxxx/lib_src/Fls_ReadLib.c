...
/* Deviation from MISRA-C:2004 rule 16.7, MISRA-C:2012 Rule-8.13.
   Justification: This implementation is specified by AUTOSAR specification. */
/* PRQA S 3673 4 */
FUNC(Std_ReturnType, FLS_CODE) Fls_ReadLib
(
...
)
{
    ...
    /* Fls099: check that driver is initialized -> FLS_E_UNINIT, E_NOT_OK */
    if( Fls_DriverState == MEMIF_UNINIT )
    {
        /* report DET error */
        (void)Fls_ReportError( FLS_MODULE_ID,
                               FLS_INSTANCE_ID,
                               ( ReadWithoutBlankCheck == FLS_FALSE )? FLS_ID_READ: FLS_ID_READIMMEDIATE,
                               FLS_E_UNINIT );

        /* set E_NOT_OK for return value  */
        Ret = E_NOT_OK;
    }
    /* Fls097: check that SourceAddress is valid -> FLS_E_PARAM_ADDRESS, E_NOT_OK */
    ...
    /* Fls098: check that Length and SourceAddress + 
    ...
    /* Fls158: check TargetAddressPtr for not NULL -> FLS_E_PARAM_DATA, E_NOT_OK */
    ...
    else
    {
        ...
        /* Fls100: check that driver is not BUSY -> FLS_E_BUSY, E_NOT_OK */
        ...
        else
        {
            ...
            /* Fls337: copy given parameters internally, initiate a read job */
            /* Fls338: set driver state to MEMIF_BUSY */
            /* Fls339: set job result to MEMIF_JOB_PENDING */
            ...
            Fls_JobRequest.ReadWithoutBlankCheck = ReadWithoutBlankCheck;
            Fls_JobRequest.CurrentJob            = FLS_READ_WAIT;

            /* set Fls_MainFunctionTime to MainFunction time-limit for a read job */
            /* check FlsSub status */
            ...
            if( Ret == FLS_STATUS_IDLE )
            {
                ...
                /* PRQA S 4394 1 */
                Fls_JobRequest.Fls_MainFunctionTime = FLS_MAX_TIME_READ;
            }
            else
            {
                /* Deviation from MISRA-C:2004 rule 10.3, MISRA-C:2012 Rule-10.8.
                   ...
                   because the time-limit is always less than 0x7FFFFFFF. */
                /* PRQA S 4394 1 */
                Fls_JobRequest.Fls_MainFunctionTime = FLS_MAX_TIME_READ + FLS_MAX_TIME_SECTOR_ERASE;
            }

            /* exit exclusive area */
            Fls_ExitCritiSec();

            /* Fls340: set E_OK for return value  */
            Ret = E_OK;
        }
    }
