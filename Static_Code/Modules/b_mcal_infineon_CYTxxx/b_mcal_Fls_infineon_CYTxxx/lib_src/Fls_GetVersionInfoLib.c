 ...
 ** \return     none
 */
FUNC(void, FLS_CODE) Fls_GetVersionInfoLib
(
    P2VAR(Std_VersionInfoType, AUTOMATIC, FLS_APPL_DATA) VersioninfoPtr
)
{
    /* Fls363: check pointer for not being a NULL pointer */
    if( VersioninfoPtr == NULL_PTR )
    {
        /* report DET error */
        (void)Fls_ReportError( FLS_MODULE_ID,
                               FLS_INSTANCE_ID,
                               FLS_ID_GETVERSIONINFO,
                               FLS_E_PARAM_POINTER
        );
    }
    else
    {
        /* Return the version information, vendor and modul id */
