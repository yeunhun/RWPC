...
** 1.12.2    14-Jan-2020   SM Kwon        #21195                              **
** 1.12.0    31-May-2019   SM Kwon        #17202                              **
...
#define CANSM_INTERNALTYPES_AR_RELEASE_MAJOR_VERSION         4
#define CANSM_INTERNALTYPES_AR_RELEASE_MINOR_VERSION         0
#define CANSM_INTERNALTYPES_AR_RELEASE_REVISION_VERSION      3

/* Software Version Information */
#define CANSM_INTERNALTYPES_SW_MAJOR_VERSION                 1
#define CANSM_INTERNALTYPES_SW_MINOR_VERSION                 12
...
#define CANSM_BOR_L1TOL2                          (LpStaticNetwork->ddBorCounterL1ToL2)
#else
...
#define CANSM_BOR_TIMEL1                          (LpStaticNetwork->ddCanSMBorTimeL1)
#define CANSM_BOR_TIMEL2                          (LpStaticNetwork->ddCanSMBorTimeL2)
#define CANSM_BOR_TIME_TXENSURED                  (LpStaticNetwork->ddBorTimeTxEnsured)
#else
...
#define CANIF_SETCONTROLLERMODE(ControllerID, ControllerMode)\
  {\
    ...
    do\
    {\
      LddResult |= CanIf_SetControllerMode(CanSM_GaaStaticController[LucControllerIndex], \
        ControllerMode);\
      --LucNoOfController;\
      ++LucControllerIndex;\
...
#define CANIF_CHANGEBAUDRATE(ControllerID, Baudrate)\
  {\
    ...
    do\
    {\
      LddResult |= CanIf_ChangeBaudrate(CanSM_GaaStaticController[LucControllerIndex], Baudrate);\
      --LucNoOfController;\
      ++LucControllerIndex;\
    } while (LucNoOfController != CANSM_ZERO);\
  }
...
#define CANIF_SETPDUMODE(ControllerID, PduMode)\
  {\
    ...
    do\
    {\
      LddResult |= CanIf_SetPduMode(CanSM_GaaStaticController[LucControllerIndex], PduMode);\
      --LucNoOfController;\
      ++LucControllerIndex;\
    } while (LucNoOfController != CANSM_ZERO);\
  }
...
#define CANIF_GETTXCONFIRMATIONSTATE(ControllerID)\
  {\
    ...
    do\
    {\
      if(LenRetVal != CANIF_NO_NOTIFICATION)\
      {\
        LenRetVal= CanIf_GetTxConfirmationState(CanSM_GaaStaticController[LucControllerIndex]);	\
      }\
      --LucNoOfController;\
      ++LucControllerIndex;\
    } while (LucNoOfController != CANSM_ZERO);\
...
#define CANSM_ELEVEN                              ((uint8) 0x0B)
#define CANSM_THIRTEEN                            ((uint8) 0x0D)
...
typedef struct 
{
  P2FUNC(void, CANSM_CODE, pTransFuncPtr) (NetworkHandleType LddNetworkHandle);
} CanSM_TransFuncPtr;
...
#include "MemMap.h"
extern CONST(CanSM_ConfigType, CANSM_CONST) CanSM_GaaConfig[CANSM_NUMBER_OF_CONFIGURATION];
...
#define CANSM_START_SEC_CONST_UNSPECIFIED
#include "MemMap.h"
extern CONST(uint8, CANSM_CONST) CanSM_GaaInternalController[CANSM_NUMBER_OF_CONFIGURATION][CANSM_LAST_CONTROLLER_HANDLE+1];
#define CANSM_STOP_SEC_CONST_UNSPECIFIED
#include "MemMap.h"
...
#define CANSM_START_SEC_CONST_UNSPECIFIED
#include "MemMap.h"
extern CONST(NetworkHandleType, CANSM_CONST) CanSM_GaaInternalNetwork[CANSM_NUMBER_OF_CONFIGURATION][CANSM_LAST_NETWORK_HANDLE+1];
#define CANSM_STOP_SEC_CONST_UNSPECIFIED
#include "MemMap.h"
...
#define CANSM_START_SEC_CONST_UNSPECIFIED
#include "MemMap.h"
extern CONST(uint8, CANSM_CONST) CanSM_GaaStaticController[CANSM_NUMBER_OF_STATIC_CONTROLLER];
#define CANSM_STOP_SEC_CONST_UNSPECIFIED
#include "MemMap.h"
...
extern CONST(NetworkHandleType, CANSM_CONST) CanSM_GaaStaticTrvToNw[CANSM_NUMBER_OF_CONFIGURATION][CANSM_LAST_TRANSCEIVER_HANDLE+1];
