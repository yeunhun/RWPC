...
** 2.4.0     07-Feb-2018   SM Kwon     Internal Redmine #14067, #15053, #15870**
...
#define COM_TP_CBK_C_SW_MINOR_VERSION  7
  ...
  COM_VAR) RxBufferSizePtr)
{
  ...
  COM_DET_TP_RXPDUID_RANGE_CHECK_FLAG(ComRxPduId, COMSERVICEID_STARTOFRECEPTION);
  ...
  P2VAR(PduLengthType, AUTOMATIC, COM_VAR) TxDataCntPtr)
{
  ...
  #endif
  {
    ...
    /* Check if Ipdu has been started */
	/* polyspace-begin MISRA-C3:18.1, RTE:STD_LIB,IDP [Not a defect: Justify with annotations] "configured & generated memory address" */
	...
	/* polyspace-end MISRA-C3:18.1, RTE:STD_LIB,IDP [Not a defect: Justify with annotations] "configured & generated memory address" */
    else
    {
      LddReturnValue = BUFREQ_E_NOT_OK;
    }
...
#define COM_START_SEC_CODE
#include "MemMap.h"
/* polyspace-begin MISRA-C3:18.1 "Configured Memory Access index" */	
/* polyspace:begin<RTE: IDP : Not a defect : No Action Planned > configured memory access index */
        ...
        (LpRxTPIpdu->ucRxTPStatusMask)) != COM_ZERO)
        {
          ...
          if(NTFRSLT_OK == Result)
          {
            ...
            {      
              ...
              LblCalloutFlag = Com_CallRxIpduCallout(ComRxPduId, 
              (PduInfoType *)&LddPduCalloutInfo);
            }
...
/* polyspace-end MISRA-C3:18.1 "Configured Memory Access index" */	
      ...
      COM_FALSE)
    {
      ...
      if((uint8)NTFRSLT_OK == Result)
      {
        ...
        #endif/*#if(COM_TX_IPDU_UPDATEBIT == STD_ON)*/      
        {
		  ...
		  /* polyspace +2 MISRA-C3:18.1, RTE:IDP [Not a defect: Justify with annotations] "Configured memory access index" */
...
FUNC(void, COM_CODE) Com_DoubleBufferCall(P2CONST
  (Com_RxTPIpdu, AUTOMATIC, COM_CONST) LpRxTPIpdu, 
  P2VAR(Com_RxPduInfoType, AUTOMATIC, COM_VAR) LpPduInfoPtr,
  P2CONST(uint8, AUTOMATIC, COM_CONST) LpSduPtr)
{
   if(LpRxTPIpdu->pDoubleIpduBuffer != NULL_PTR)
