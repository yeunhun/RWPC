...
#define COM_TXIPDUSEND_C_SW_MINOR_VERSION  7
...
#include "MemMap.h"
static FUNC(uint8, COM_CODE) Com_TxIpduTMSCalc
                              (PduIdType LddTxPduId, uint16 LusConfig);
...
#endif
{
  ...
  #if((COM_TX_IPDU_TMS_SIG_BASED == STD_ON)||(COM_TX_IPDU_MINDELAY_ENABLED == STD_ON))
  /* Local variable to store Ipdu config */
  uint8 LucIpduConfig;
  #endif
  ...
  {
    LucTransmitFlag = Com_TxIpduTMSCalc(LddTxPduId, LusConfig);
  ...
  if((LucTransmitFlag & (COM_ONE)) != COM_FALSE)
  {
    ...
    if((((LpTxMode->ucTxMode) >= COM_TX_MIXEDNTIMES_MODE) &&
        ...
        ((LusConfig & COM_TX_SIGPROP_WOREPT_CONFIG_MASK) !=
    ...
    #endif
    #if (COM_TX_IPDU_GW == STD_ON)
    if(((LusConfig & COM_TX_SIG_GW_CONFIG_MASK) != COM_FALSE))
    {
  ...
  LpTxSignal, P2CONST(void, AUTOMATIC, COM_CONST) SignalDataPtr)
{
  ...
  if((LucSigConfig & COM_TX_SIG_FILTER_CONFIG_MASK) != COM_FALSE)
  {
    ...
    /* Reset the previous value in MASKED_NEW_DIFFERS_MASKED_OLD filter, 
	  ...
	  /* polyspace:begin<RTE: NIV : Not a defect : No Action Planned > initialized variable */
	  /* polyspace:begin<RTE: IDP : Not a defect : No Action Planned > Pointer is not a NULL and valid */
	  /* polyspace:begin<MISRA-C3:18.1 : Not a defect : No Action Planned > Genderated array index in range */
    ...
    /* polyspace:end<RTE: COR : Not a defect : No Action Planned > */
	/* polyspace:end<RTE: NIP : Not a defect : No Action Planned > */
	/* polyspace:end<RTE: NIV : Not a defect : No Action Planned > */
	/* polyspace:end<RTE: IDP : Not a defect : No Action Planned > */
	/* polyspace:end<MISRA-C3:18.1 : Not a defect : No Action Planned > */
                                ...
                                (PduIdType LddTxPduId, uint16 LusConfig)
{
  ...
  LusTMSResult = (LusConfig & COM_TX_SIG_TMS_STATUS_MASK);
  /* Get the pointer to Tx Ipdu */
  LpTxIpdu = &Com_GaaTxIpdu[LddTxPduId];  
  /* Get the pointer to True Tx mode */
  LpTxMode = &Com_GaaTxMode[LpTxIpdu->ddTrueTxModeIndex];
  ...
  else if(COM_FALSE == LusTMSResult)
  {
  ...
  {
    if(((LusConfig & COM_TX_SIG_PROPERTY_CONFIG_MASK) >
        COM_TX_SIG_PENDING_PROPERTY) &&
