...
** 2.7.1     07-Jan-2020   SM Kwon     Internal Redmine #27557                **
** 2.7.0     31-Dec-2020   SM Kwon     Internal Redmine #27452                **
** 2.6.0     10-Sep-2020   SM Kwon     Internal Redmine #23914, #25614        **
** 2.5.1     23-Jul-2020   SM Kwon     Internal Redmine #24834                **
** 2.5.0     31-Jan-2020   SM Kwon     Internal Redmine #21262                **
...
#define COM_TXSIG_C_SW_MINOR_VERSION  7
...
#include "MemMap.h"
static FUNC(uint16, COM_CODE) Com_TxSigGrpModeSelect
...
#define COM_START_SEC_CODE
#include "MemMap.h"	
static FUNC(uint8, COM_CODE) Com_SendSignalService
...
#define COM_START_SEC_CODE
#include "MemMap.h"
static FUNC(void, COM_CODE) Com_UpdateShadowSignalService
          (Com_SignalIdType SignalId,
           P2CONST(void, AUTOMATIC, COM_CONST) SignalDataPtr);
#define COM_STOP_SEC_CODE
#include "MemMap.h"
...
#define COM_START_SEC_CODE
#include "MemMap.h"
static FUNC(boolean, COM_CODE) Com_TxSigValidationCheck
    ...
    if(LucReturnValue != (uint8)E_NOT_OK)
    {
      ...
      if(LpTxIpdu->ucIpduType == COM_DYN_IPDU)
      {
        ...
        #endif      
        {
          ...
          if(Com_GaaTxIpduStatus[LpTxSignal->ddTxIpduIndex] != (uint16)COM_ZERO)
          {
...
FUNC(uint8, COM_CODE) Com_InvalidateSignal(Com_SignalIdType SignalId)
{
  ...
  #endif
  {
  	if(SignalId < COM_TX_SIGNAL_COUNT)  
	{
      #if(COM_DEV_ERROR_DETECT == STD_ON)
...
**                                                  Det_ReportError()         **
*******************************************************************************/
/* polyspace:begin<MISRA-C3:8.7:Not a defect:Unset> function will be invoked from upper layer */
...
#include "MemMap.h"
#endif
/* polyspace:end<MISRA-C3:8.7:Not a defect:Unset> function will be invoked from upper layer */
...
#define COM_START_SEC_CODE
#include "MemMap.h"
/* polyspace:start<MISRA-C3:18.1:Not a defect:Justify with annotations> configured memory access index */
FUNC(uint8, COM_CODE) Com_SendSignalGroup (Com_SignalGroupIdType SignalGroupId)
{
  ...
  uint16 LusSigGrpConfig;
  /* Local variable to store signal group property */
  uint16 LusSigGrpProperty;
    ...
    #endif
    {
     ...
     /* Get the pointer to Tx IPdu structure */
      ...
      #endif
      {
        ...
        /* Check whether update bit is configured */
        if((LusSigGrpConfig & COM_TX_SIGGRP_UB_CONFIG_MASK) != COM_FALSE)
        {
          /* Enter protection area */
          SchM_Enter_Com_TX_IPDU_BUFFER_PROTECTION_AREA();
          /* Set the update bit */
        ...
        #endif
        #if(COM_TX_SIGGROUP_TMS == STD_ON)
        LusSigGrpConfig = Com_TxSigGrpModeSelect(LpTxSigGrp);
        #endif
        /* Set return value to COM_SERVICE_NOT_AVAILABLE */
        LucReturnValue = (uint8)COM_SERVICE_NOT_AVAILABLE;
        /* Check whether I-PDU group is started */
        if(Com_GaaTxIpduStatus[LpTxSigGrp->ddTxIpduIndex] != (uint16)COM_ZERO)
        {
          /* Set return value to E_OK */
          LucReturnValue = (uint8)E_OK;
          #if(COM_TX_SIGGROUP_TRIGONCHANGE == STD_ON)
          ...
          #endif
          {
            if((LusSigGrpProperty) > COM_TX_SIGGRP_PENDING_PROPERTY)
            {
              #if ((COM_TX_IPDU_TMS_SIG_BASED == STD_ON) || \
              (COM_TX_IPDU_REPETITION == STD_ON) || \
              ((COM_TX_IPDU_MINDELAY_ENABLED == STD_ON) \
              ...
              (COM_TX_IPDU_GW == STD_ON) || \
              (COM_TX_IPDU_DM == STD_ON))
              (void)Com_TxRequest(LpTxSigGrp->ddTxIpduIndex, LusSigGrpConfig);
    ...
    {      
      ...
      #endif
      {
        ...
        if(LusSigGrpProperty >= COM_TX_SIGGRP_TRIGGERED_ON_CHANGE)
        {
          ...
          if((LusSigGrpProperty & COM_TX_GRPSIG_TRIGGERED_ON_CHANGE) != COM_FALSE)
          {
            LblReturnFlag = Com_PackAndValidateGrpSigArray(LpTxSigGrp, LpTxPack, SignalGroupArrayPtr, LpTxPack->pWrBuffer);
          }
          else
          #endif
          {
        ...
        if((LusSigGrpConfig & COM_TX_SIGGRP_UB_CONFIG_MASK) != COM_FALSE)
        {
          ...
          SchM_Enter_Com_TX_IPDU_BUFFER_PROTECTION_AREA();
          /* Set the update bit */
		  /* polyspace +1 MISRA-C3:18.1, RTE:IDP [Not a defect: Justify with annotations] "Configured memory access index" */
          *(LpTxSigGrp->pUbit) |= LpTxSigGrp->ucUbitMask;
          /* Exit protection area */
          SchM_Exit_Com_TX_IPDU_BUFFER_PROTECTION_AREA();
        }
...
#include "MemMap.h"
/* polyspace:begin<MISRA-C3:18.1:Not a defect:Justify with annotations> configured memory access index */
static FUNC(void, COM_CODE) Com_UpdateShadowSignalService(Com_SignalIdType SignalId,
           P2CONST(void, AUTOMATIC, COM_CONST) SignalDataPtr)
{
  /* #7525 #7705 start */
  ...
  */
  ...
  LblTypeValueValid = Com_TxSigValidationCheck(LpTxSigInGrp->usBitSize, 
  LpTxSigInGrp->ucType, SignalDataPtr);
		
  if(LblTypeValueValid == COM_FALSE)
  {
...
#include "MemMap.h"
/* polyspace:begin<MISRA-C3:18.1:Not a defect:Justify with annotations> configured memory access index */
static FUNC(uint8, COM_CODE) Com_SendSignalService (Com_SignalIdType SignalId,
  P2CONST(void, AUTOMATIC, COM_CONST) SignalDataPtr)
{
  /* Pointer to Tx signal structure */
  P2CONST(Com_TxSignal, AUTOMATIC, COM_CONST) LpTxSignal;
  ...
  adding range check for HMC specification - by Chan Kim 2013.09.11-START
  */
  LblTypeValueValid = Com_TxSigValidationCheck(LpTxSignal->usBitSize, 
  ...
  */
  ...
  #endif
  {
    ...
    buffer */
    (void)Com_PackRequestWrite(LpTxPack, SignalDataPtr);
    #if((COM_TX_SIGNAL_UB == STD_ON) || (COM_TX_IPDU_TMS_SIG_BASED == STD_OFF))
    /* Get the signal configuration into local variable */
    LucSigConfig = LpTxSignal->ucTxSigConfig;
    #endif
    #if(COM_TX_SIGNAL_UB == STD_ON)
                       ...
                       (P2CONST(Com_TxSigGrp, AUTOMATIC, COM_CONST) LpTxSigGrp)
{
  ...
  LusSigGrpConfig = LpTxSigGrp->usTxSigGrpConfig ;
  /* Check whether update bit is configured */
  if((LusSigGrpConfig & COM_TX_SIGGRP_TMS_CONFIG_MASK) != COM_FALSE)
  {
    LblResult = COM_FALSE;
    ...
    do
    {
      ...
      if(*LpFilterStatus != COM_ZERO)
      {
        ...
        LusSigGrpConfig |= COM_TX_SIGGRP_TMS_STATUS_MASK;
        /* Enter protection area */
        SchM_Enter_Com_TX_TMS_STATUS_PROTECTION_AREA();
...
#define COM_START_SEC_CODE
#include "MemMap.h"
 /* polyspace-begin MISRA-C3:D4.14,18.1, RTE:IDP [Not a defect: Justify with annotations] "Configured memory access index" */	
    ...
    switch(signalType)
    {
      ...
      case COM_SIG_TYPE_SINT8:
        ...
        else
        {
          if(signalSize != ((uint16)COM_EIGHT)) 
          {
            ...
            LscSigTypeSint8VarUP = ((sint8)LucSigTypeUint8Var - (sint8)COM_ONE);
      ...
      case COM_SIG_TYPE_SINT16:
        ...
        else
        {
          if(signalSize != ((uint16)COM_SIXTEEN)) 
          {
            LusSigTypeUint16Var = (uint16)COM_ONE << (signalSize-(uint16)COM_ONE);
            LscSigTypeSint16VarLO = -(sint16)(LusSigTypeUint16Var);
            LscSigTypeSint16VarUP = ((sint16)LusSigTypeUint16Var - (sint16)COM_ONE);
      ...
      case COM_SIG_TYPE_SINT32:
        ...
        else
        {
          if(signalSize != ((uint16)COM_THIRTY_TWO)) 
          {
            LulSigTypeUint32Var = (uint32)COM_ONE << (signalSize-(uint16)COM_ONE);
            LscSigTypeSint32VarLO = -(sint32)(LulSigTypeUint32Var);
            LscSigTypeSint32VarUP = ((sint32)LulSigTypeUint32Var - (sint32)COM_ONE);
...
/* polyspace-end MISRA-C3:D4.14,18.1, RTE:IDP [Not a defect: Justify with annotations] "Configured memory access index" */	
#define COM_STOP_SEC_CODE
#include "MemMap.h"  
#endif
/* polyspace:end<MISRA-C3:18.1:Not a defect:Justify with annotations> configured memory access index */
