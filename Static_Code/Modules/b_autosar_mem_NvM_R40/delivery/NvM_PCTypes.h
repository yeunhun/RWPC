...
{\
  ...
  blkStatus = NvM_GaaRamStatus[(BlockId)];\
  NvM_GaaRamStatus[(BlockId)] = (blkStatus & NVM_NOT_ERROR_BIT_MASK) | (uint16)(NewReqResult);\
  SchM_Exit_NvM_RAM_STATUS_PROTECTION();\
} while(0)

/* polyspace +3 MISRA-C3:19.7:Not a defect:Justify with annotations> Not a defect */
#define NVM_GET_JOB_RESULT(BlockId) ((NvM_RequestResultType)(NVM_ERROR_BIT_MASK & NvM_GaaRamStatus[(BlockId)]))

#define NVM_RESERVED_BLOCK_ID             (0U)

#define NVM_IS_BIT_SET_BLOCKSTATUS(BlockIndex, Mask)               ((NvM_GaaRamStatus[NvM_GddBlockIdx] & (uint16)(Mask)) != (uint16)0U)
#define NVM_IS_BIT_UNSET_BLOCKSTATUS(BlockIndex, Mask)             ((NvM_GaaRamStatus[NvM_GddBlockIdx] & (uint16)(Mask)) == (uint16)0U)
#define NVM_SET_BLOCKSTATUS(BlockIndex, Mask)                      do{(NvM_GaaRamStatus[NvM_GddBlockIdx] |= (uint16)(Mask));} while(0)
#define NVM_CLR_BLOCKSTATUS(BlockIndex, Mask)                      do{(NvM_GaaRamStatus[NvM_GddBlockIdx] &= (uint16)(~(uint16)(Mask)));} while(0)
#define NVM_RST_BLOCKSTATUS(BlockIndex, Mask)                      do{(NvM_GaaRamStatus[NvM_GddBlockIdx] = (uint16)(Mask));} while(0)
