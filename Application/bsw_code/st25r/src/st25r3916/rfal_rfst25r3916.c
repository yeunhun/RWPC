...
uint16 indAmp_value = 0u;
uint16 indAmp_refer = 0u;
 ...
 *  Wake-Up Mode                                                               *
 *******************************************************************************/
extern uint8 techDctCnt;
uint8 inPha_test = 0u;
uint8 inPha_test1 = 0u;
...
ReturnCode rfalWakeUpModeStart( const rfalWakeUpConfig *config )
{
    ...
    if( config == NULL )
    {
        gRFAL.wum.cfg.period           = RFAL_WUM_PERIOD_200MS;	//RFAL_WUM_PERIOD_200MS -> RFAL_WUM_PERIOD_500MS;
        gRFAL.wum.cfg.irqTout          = false;
		gRFAL.wum.cfg.swTagDetect      = false;
		
        gRFAL.wum.cfg.indAmp.enabled   = true;
        gRFAL.wum.cfg.indPha.enabled   = false; //false -> true;
		...
		gRFAL.wum.cfg.indPha.delta     = 2U;		//added.....to do
		gRFAL.wum.cfg.indPha.reference = RFAL_WUM_REFERENCE_AUTO;	//added....to do
        ...
        if( st25r3916CheckReg( ST25R3916_REG_IO_CONF2, ST25R3916_REG_IO_CONF2_aat_en, ST25R3916_REG_IO_CONF2_aat_en ) )
        {
			...
			gRFAL.wum.cfg.indPha.autoAvg  = false;
			gRFAL.wum.cfg.indPha.aaWeight = RFAL_WUM_AA_WEIGHT_16;
                ...
                if( gRFAL.wum.cfg.swTagDetect )
                {
                    ...
                    if( gRFAL.wum.cfg.indAmp.enabled )
                    {
                        ...
                        if( gRFAL.wum.cfg.indAmp.autoAvg )
                        {
                        	...
                        	indAmp_refer = gRFAL.wum.cfg.indAmp.reference;
						}
                    }
